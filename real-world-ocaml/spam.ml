...
let truncate ?(leng=15) str = match str with
  | _ when (String.length str) > leng -> String.sub str 0 leng
  | _ -> str

(* buildmap builds up a map of tokens and the frequency count of that
 * token in a give file or string. Takes a lexbuf as argument *)
let rec buildmap startmap (lb : Lexing.lexbuf) =
  let next_tok = try Some (truncate (Spamlex.tokens lb)) with End_of_file -> None
...
(*
 * Returns the first n element of a sorted list in a somewhat crude way,
 * by converting the list into an array and then taking a subarray.
 *)
let top_n n lst = try
...
let paul_graham word goodMap badMap ngood nbad =
  ...
  | _,_ ->
    let g = try 2 * (StringMap.find word goodMap)
      with Not_found -> 0 in
    let b = try StringMap.find word badMap
      with Not_found -> 0 in
    let numerator = try min 1.0 ((float_of_int b) /. (float_of_int nbad))
      with Division_by_zero -> 1.0 in
    let denom = try (min 1.0 ((float_of_int g) /. (float_of_int ngood)))
      with Division_by_zero -> 1.0 in
...
(* Calculate the probability of a given email being spam by finding the proabbilities
 * of each token being spam, taking the top 20 tokens and combining them. *)
let calc_email_prob lbuf =
  ...
  let top_vals =
    top_n 20 (List.rev (List.sort compare (StringMap.fold (fun x y z -> y :: z) scored []))) in
